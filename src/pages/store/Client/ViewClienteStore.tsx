    ...
    return (
      <div className="max-w-6xl mx-auto p-5 sm:p-8">
        ...
        <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white/90 dark:bg-gray-900/60 shadow-sm p-6 mb-8">
          ...
          {client?.rut ? (
            <div
              onClick={() => window.open(client?.rut.url, "_blank")}
              className="flex items-center justify-between cursor-pointer border border-gray-200 dark:border-gray-700 rounded-xl p-4 hover:bg-indigo-50/60 dark:hover:bg-indigo-900/30 transition"
            >
            ...
            <p className="text-gray-500 dark:text-gray-400">
              No hay documento de RUT disponible.
            </p>
          )}
        </div>
          ...
          {client?.addresses?.length ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {client.addresses.map((addr: any) => (
                <div
                  key={addr.id}
                  className="p-4 border border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50/50 dark:bg-gray-900/40 shadow-sm hover:shadow-md transition-all duration-200"
                >
                  <h3 className="font-semibold text-gray-800 dark:text-gray-100 mb-1">
                    {addr.name || "Dirección sin nombre"}
                  </h3>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-300">
                    {addr.zipCode && `Código postal: ${addr.zipCode}`}
                  </p>
                  {addr.reference && (
                  ...
                  <div className="flex items-center justify-between mt-3 text-sm">
                    ...
                    >
                      {addr.isDefault ? "Predeterminada" : "Secundaria"}
                    </span>
                    <span
                      className={clsx(
                        "px-2 py-1 rounded-md font-medium",
                  ...
                  <p className="text-xs text-gray-500 mt-2">
                    Creada: {new Date(addr.createdAt).toLocaleDateString()} — Actualizada:{" "}
                    {new Date(addr.updatedAt).toLocaleDateString()}
                  </p>
                </div>
              ))}
            </div>
